...
c---------------------------------------------------------------------
     ...
     >        total_time, timer_read(T_transpose), 
...
c---------------------------------------------------------------------
 ...
 1000    format(//,' NAS Parallel Benchmarks 3.2 -- FT Benchmark',/)
...
c---------------------------------------------------------------------
     ...
     >           x1, x1, scratch)
            call cffts2(1, dims(1,2), dims(2,2), dims(3,2), 
     >           x1, x1, scratch)
            call timer_start(T_transpose)
            call transpose_xy_z(2, 3, x1, x2)
            call timer_stop(T_transpose)
...
!The jth block of data sent from task i is received by task j and placed in the ith b!!lock of the buffer recvbuf.

      
      do i = 1, np2
        q           = mod(me2+i,np2)
        start_send  = (ntdivnp/np)*(q)+1
        start_recv  = (ntdivnp/np)*(me2)+1
        j1 = me1*np2+q
        call shmem_put128(xout(start_recv), 
     >  xin(start_send), ntdivnp/np, j1)
      end do
...
c---------------------------------------------------------------------
      ...
      do i = 1, np2
         ...
         start_recv  = (nb)*(me2)+1
         j1 = me1*np2+q
         call shmem_put128(xout(start_recv,start_recv,start_recv), 
     >        xin(start_send,start_send,start_send), nb, j1)
      end do
...
c---------------------------------------------------------------------
     ...
     >                          xout, xin)
      call transpose_x_y_finish(dims(1,l1),dims(2,l1),dims(3,l1), 
     >                          xin, xout)
...
c     double complex, dimension(d2,d3,d1), save::xout ! not real layout but right size
      integer ierr,j1
      integer ::  q, i
      integer :: start_send, start_recv, nb
...
!      if (me .eq. 0) then
!            write (*, 29) i, chk
! 29         format (' T =',I5,5X,'Checksum chk =',1P2D22.12)
!      endif
...
c---------------------------------------------------------------------
      ...
      double precision vdata_real_d(25)
      double precision vdata_imag_d(25)
...
c---------------------------------------------------------------------
      data vdata_real_d / 5.122230065252D+02,
     $     5.120463975765D+02,
     $     5.119865766760D+02,
     $     5.119518799488D+02,
     $     5.119269088223D+02,
     ...
     $     5.118822370068D+02 /
      ...
      data vdata_imag_d / 5.118534037109D+02,
     $     5.117061181082D+02,
     $     5.117096364601D+02,
     $     5.117373863950D+02,
     $     5.117680347632D+02,
     ...
     >     nt .eq. 6) then
         ...
         do i = 1, nt
            ...
            if (.not.(abs(err) .le. epsilon)) goto 100
...
c     err = (aimag(sums(i)) - vdata_imag_c(i)) / vdata_imag_c(i)
 ...
 130     continue
     ...
     >        nt .eq. 25) then
         class = 'D'
         do i = 1, nt 
            err = (dble(sums(i)) - vdata_real_d(i)) / vdata_real_d(i)
            if (.not.(abs(err) .le. epsilon)) goto 140
            err = (dimag(sums(i)) - vdata_imag_d(i)) / vdata_imag_d(i)
c     err = (aimag(sums(i)) - vdata_imag_d(i)) / vdata_imag_d(i)
            if (.not.(abs(err) .le. epsilon)) goto 140
         end do
         verified = .TRUE.
 140     continue
